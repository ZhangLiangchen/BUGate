...
2. **使用规范** — 默认模式是**导入模式（Imported Governance Layer）**：agent runtime 打开
...
违背其存在理由——"诚实不能靠提醒，只能靠机器可校验的产物与物理门禁来强制"（白皮书 §1）。
...
| 形态 | 含义 | 状态 |
|---|---|---|
| **内嵌模式**（Embedded） | BUGate 手工融合在起源 SUT 测试仓内 | **已冻结**（TRANSITION_PROTOCOL：frozen reference + fallback），按退出判据退役 |
...
- **R2（必须）** 导入模式下，SUT profile 与 BUGate 配置**提交进 SUT 仓**，与被守护的测试代码
  同库、同评审、同历史。**治理配置必须与被治理物同库同审**——"本地未提交 profile 指针"
  约定仅适用于工作台模式（§2.3 / R7）。
...
- **R7（必须）** 工作台模式下，SUT 测试工作区以 symlink 挂载 + 本地**未提交**的 `profile:`
  指针接入（README "Physical layout" 的既有约定）；两仓保持完全独立的历史、remote 与生命周期。
- **R8（禁止）** 把工作台模式作为面向使用者的主叙事、教程默认路径或生产使用方式。它是
...
- **✗ 宿主倒置作为叙事或默认用法**："SUT 挂载在 BUGate 之下"会导致治理契约（profile）
...
归因口径（维持白皮书既定表述，开源时保留）：BUGate "absorbs useful ideas … without
...
| 事项 | 内容 |
|---|---|
| README | "Physical layout / Quickstart" 改写：导入模式为默认叙事；symlink 挂载小节改标注为 **Core Workbench（maintainer）模式** |
...
**验收**：新读者按 README 五分钟路径建立的心智模型 = 导入模式；工作台模式只出现在维护者小节。
...
- **`bugate init <sut-repo>`**：合并/写入双 runtime hook 接线、建立或复制 skills、脚手架
  ...
  （re-trust）提示。安装步骤基线：起源工作区的《拆分接缝清单》（2026-06-11）§5–6 已给出
  完整清单（kit + 单一项目配置 + re-trust 注意事项）。
- **双通道**：一端 runtime 支持插件系统的，打包为 plugin（skills + commands + agents + hooks
  可整体入插件，hook 经插件根变量调引擎）；不支持插件系统的一端由 `bugate init` 脚手架补位。
- **引擎分发形态**（三选一，倾向 ③）：
...
- **现状**：哨兵式根发现（`AGENTS.md` + `.shared/`）把"引擎根"与"工作区根"捆绑在一起，
  恰好匹配工作台模式与起源工作区，但不匹配任意 SUT 仓。
...
- **白皮书 §13 路线**（左移 PRD / 设计 / TDD，BUGate Loop 元门禁）依赖导入形态横向铺开——
  一个母仓不可能承载全组织被治理的工作流。**本文的宿主翻转是这些路线的前置条件。**

### 5.5 兼容性声明

- **与 ADR-BUGATE-001**：四部分模型（Core / Profile / Mounted Workspace / Runtime）与
  "谁是项目根"**正交**；ADR 原文即允许 profile 位于 mounted workspace 一侧。本文不修改
  ADR 任何条款。
- **与 TRANSITION_PROTOCOL**：三桶分类、asymmetric strangler-fig、transition-gap ledger、
  退出判据全部保留；本文仅把退出终态明确为"起源工作区以导入模式引入 kit，替换冻结内嵌栈"。
- **与 README**：README "Physical layout — symlink the SUT in" 小节自本文起限定为
  工作台模式约定（P0 落实改写）。
...
| 日期 | 事件 |
|---|---|
| 2026-06-11 | 起源工作区《拆分接缝清单》首次给出目标形态：**可导入 kit + SUT 仓内单一项目配置**（即本文的导入模式） |
| 2026-06-16 | ADR-BUGATE-001 确立四部分解耦模型与 Promotion Rule |
| 2026-06-17 起 | core 去 SUT 化：中性 skill、profile 机制、de-SUT guard、demo-sut |
...
抽取期为自托管所需而形成的"SUT symlink 挂进 BUGate"用法，功不可没且继续保留——但自本文起
定名为工作台模式，限定于 §2.3 的四类维护者活动。
