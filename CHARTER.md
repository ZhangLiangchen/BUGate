...
| 平面 | 归属 | 负责 |
|---|---|---|
| **治理控制面**（control plane） | **BUGate** | PRD 健康度、业务命题与 oracle、testability 决策、case inventory、可读用例、adversarial review、物理写门禁、角色路径隔离、可审计生命周期治理、Memory 治理与回写 |
...
| **强制面** | 物理写入门 + 语义门脚本 + 角色路径隔离 + lifecycle receipt 校验 + de-SUT CI | Policy-as-Code / **准入控制（admission control）** | 无——BUGate 的差异化 |
| **记忆面** | Memory Bus（无证据 confirmed 拒写）+ strict handoff/acceptance 锚点 + 04/05 回写 + 经验晋升 | Institutional memory / 学习闭环 | 学习型系统有，但不与门禁耦合 |

强制面值得一个精确类比：挂在 PreToolUse 上的物理写门就是 **admission webhook 的 agent 版**
——拦截写操作、按策略（产物 `gate_status`，以及启用 required role governance 时的
角色、session、handoff/acceptance receipt 与 hash drift）裁决放行或拒绝、fail-closed。
...
1. **换 harness 测试**：同一份规则源（`.shared/skills/bugate/` + `scripts/`）同时运行在两个
   runtime 上。Claude 把写门接到 `Edit|Write`，把角色路径守卫单独接到
   `Read|Edit|Write`；Codex 在 `apply_patch` 上执行同一组写门与角色守卫。
   `SessionStart` 同时执行 best-effort Memory recall 与角色治理状态检查。适配层只负责把
   runtime 事件路由到同一组 SUT-neutral scripts；换 runtime 不会重写治理内核。
2. **代码占比测试**：只有少量 adapter scripts 解析 hook 载荷；绝大多数 stdlib gate、
   orchestrator、Memory 与 evidence 逻辑都可在没有任何 agent 的 CI 中独立运行。这里刻意不
   写死脚本数量，避免新增治理面后让定位文档与实际 surface 漂移。
...
| 强制面（fail-closed 门禁） | ✗ 靠 agent 自觉 | ✗ 靠 skills 说服 | ✓ 物理写门 + 语义门 + 角色路径隔离 + 可审计生命周期 receipt + de-SUT CI |
