...
- **Fresh install and update are separate.** Until a packaged installer
  ...
  imports use `bugate-update`: v0.3.x bootstraps from an unpacked v0.4.2
  `scripts/bugate_update.py`; v0.4+ uses the vendored `bin/bugate-update`.
...
- **The dual-agent CLIs are optional; the memory bus is required.** The
  ...
  anomaly. Ordinary recall/notes/Stop and per-edit role checks stay
  best-effort/local. In Wave 7 `memory_mode: required`, a transient outage
  intentionally fail-closes only the next role transition and cannot publish
  an unlocking receipt. Nothing in core mounts or depends on a SUT repository.
...
| Roll back one committed update | distribution | `bin/bugate-update rollback` | `[target]`, `--vendor-dir`, `--transaction <32-hex-id>`, `--json` | Mutating, transaction-specific reversal followed by `verify`; never guesses a transaction and never rolls back a separately reviewed profile change. |
...
| Auditable lifecycle state machine — approve, hand off, accept, complete, inspect, and launch distinct role sessions | 7 | `role_governance.py` / `bin/bugate-role` | `status`, `verify [--strict-memory]`, `approve`, `handoff`, `accept`, `complete`, `session-start`, `run --role … -- <command>` | `role_governance`, `BUGATE_AGENT_ROLE`, `BUGATE_SESSION_ID`, `BUGATE_AGENT_RUNTIME` | `mode: off` preserves v0.3.x; `advisory` warns; `required` fail-closes malformed config, wrong/unset identity, missing/stale receipts, and drift | `bin/bugate-role run --role designer -- codex` |
| Role-evidence hook — enforce phase ownership and locally verify profile/artifact/implementation/receipt hashes; deny direct evidence edits | 7 | `check_role_evidence.py` | path args or Claude/Codex hook JSON on stdin | same Wave 7 keys; `evidence_dir` defaults to `00_role_evidence` | `mode: off` no-op; no per-edit Memory call; both this guard and `check_bugate.py` must pass for Layer 4 | `echo '{"tool_input":{"file_path":"tests/x.py"}}' \| python3 scripts/check_role_evidence.py` |
...
| `memory_bus.py get` | Exact-read one Memory content hash | `--id`; `--strict` makes missing/unavailable/mismatch non-zero |
| `memory_bus.py handoff --strict` | Strictly write and exact-verify a role handoff anchor | roles plus `--event`, `--uc`, `--phase`, `--transition-sha256`, `--receipt-sha256` metadata |
| `memory_bus.py accept-handoff --strict` | Exact-verify a supplied handoff ID, then write/exact-verify acceptance | `--handoff-id`, `--handoff-receipt-sha256`, roles, UC, phase, session and transition fields |
| `memory_bus.py verify-handoff --strict` | Re-audit an exact Memory/local-receipt binding | `--id`, optional `--receipt-file`, expected roles/UC/phase/hash fields |
...
`agent_roles`, `role_governance` (including phases, Memory strictness, evidence
directory, session policy, and human-acceptance artifacts), the
`falsification_*` / `prd_health_*` / `require_*` /
...
`BUGATE_AGENT_ROLE` / `BUGATE_SESSION_ID` / `BUGATE_AGENT_RUNTIME`) is documented canonically — with types, defaults, and a
