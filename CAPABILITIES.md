...
| Importer (`bugate_init.py`) — vendors the kit, links skills, merges + refreshes the BUGate hook wiring (never the repo's own hooks), scaffolds committed config + profile | The **engine checkout** (this repo, or an already-vendored kit), pointed at a target SUT repo |
