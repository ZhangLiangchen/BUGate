...
>
> For an installation that is already imported, prefer the dedicated
> [`UPDATE_PROMPT.md`](UPDATE_PROMPT.md) or the `bugate-update` skill. This
> combined prompt retains the same classifier for backward-compatible import
> routing, but an upgrade request defaults to plan-only.
...
- BUGate target release line: use `BUGATE_VERSION` if set, otherwise `0.4.5`.
  The default is usable only after that public tag/Release and its assets pass
  checksum verification; until then, explicitly select the published v0.4.4
...
- When the public v0.4.5 tag/Release exists, it becomes authoritative only if
  it has exactly three assets: `bugate-0.4.5.tar.gz`, `bugate-0.4.5.zip`, and
  `bugate-0.4.5.SHA256SUMS`. The checksum asset is mandatory; verify the
  selected archive before extraction. Until then, use the published v0.4.4
  fallback explicitly.
...
2. **Acquire the BUGate kit outside the SUT repo when required**
   ...
   - Otherwise run the equivalent of:
     ...
     ```bash
     BUGATE_VERSION="${BUGATE_VERSION:-0.4.5}"
     BUGATE_TMP="$(mktemp -d)"
...
4. **Run exactly one install/update path**
   ...
   **Lock+launcher in-repo update** (`BUGATE_ROUTE=locked-in-repo-update`):
   ...
   ```bash
   "$UPDATER" plan \
     --archive /outside/bugate-0.4.5.tar.gz \
     --checksums /outside/bugate-0.4.5.SHA256SUMS
   "$UPDATER" apply \
     --archive /outside/bugate-0.4.5.tar.gz \
     --checksums /outside/bugate-0.4.5.SHA256SUMS
   "$UPDATER" verify
