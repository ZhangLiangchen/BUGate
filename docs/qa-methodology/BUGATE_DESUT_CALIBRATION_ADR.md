...
hard-coded FORBIDDEN list of the legacy SUT's vocabulary (product name,
...
- **The blockade taxed the documentation dividend.** Historical import stories, real
...
- **Industry vocabulary was collateral damage.** Generic method text could not
  ...
  to sit on the legacy SUT's list, though those words identify no SUT.
- **The list was baked into engine source**, so every new SUT inherited the
  legacy SUT's vocabulary as its own forbidden list — itself a form of the
...
2. **Identity terms** — SUT/product/internal-system/person/account names.
   **Forbidden by default**; narrative/provenance contexts (case studies,
   migration history, real import tutorials) are
...
(`CHARTER.md`, which never ships in the vendored kit). An imported SUT repo's
own files are **never** the scan surface — generated imported-repo fixtures are
excluded when nested under the engine root, and in a vendored layout the SUT
repo files are simply not kit members. Files that legitimately declare the
terms (the active profile/config, term-list fixtures) are excluded likewise.
...
built-in hygiene; the legacy fixture (the legacy SUT identity can never seep
back); and the meta-test (`tests/test_desut_guard.py`) proving every red/green
verdict — including the second-SUT defense — with negative controls. The legacy
SUT vocabulary now exists only in the fixture and in that SUT's own profile —
never in engine source.

## Consequences

- Historical import stories remain writable under explicit narrative boundaries:
  `docs/case-studies/origin-sut-import.md` may name the legacy SUT under the
  allowlisted case-study directory, and CI stays green — the documentation
  dividend this calibration was for.
