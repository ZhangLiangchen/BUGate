...
- [ ] 项目根目录已创建 `.ai/` 与 `.claude/` 目录结构（参见 METHOD.md §10.1）
- [ ] `.claude/agents/` 下已配置 5 个核心 agent：prd-health-checker, prd-reader-a/b/c, business-model-builder
- [ ] `.ai/scripts/` 下已有 schema 校验脚本
- [ ] PRD 文档可访问（路径填入 agent 配置）
- [ ] 至少一位开发与一位业务方愿意配合（每周 1-2 小时即可）

未就位的，先按 `templates/` 下的初始化脚本配置好，再开始 Wave 0。
...
2. 三个 agent 使用不同的提示策略：
   ...
   **AI-A（标准提取）**：
   ...
   严格按 proposition.schema.yaml 输出。每条命题必须有 source 与 source_quote。
...
1. 跑命题聚类脚本：
   ```bash
   .ai/scripts/merge_propositions.py \
     --input .ai/raw-propositions/ \
     --output .ai/audit/clusters.yaml
   ```

   脚本会按 statement 语义相似度聚类，输出四档：
   - `consensus`：多 AI 一致
   - `partial`：部分缺失
   - `conflict`：对立命题
   - `orphan`：孤证
