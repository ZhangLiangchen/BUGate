...
> *本期详细覆盖 Wave 0-3，并提供 BUGate v0.4.0 引入的 Wave 7 可审计角色执行 SOP，以及 v0.4.3 lineage 完整性操作补充；Wave 4-6/8 的方法论操作细则仍在后续版本。*
...
2. 审查 update diff、transaction ID、`verify` 与 smoke 结果，确认 SUT-owned hook/profile/
   receipt、machine lineage registry 与 Memory home 未被修改；先把完整 BUGate-owned installed projection（含 lock/hook）
   作为**第一个 commit**提交，
...
4. 再把 profile 中唯一的 `role_governance` 从 `mode: off` 替换为
   ...
   updater 不得代改 profile，也不得伪造 acceptance/handoff/receipt。Updater plan 中的
   profile `migration_required` 是 profile-schema compatibility 状态；它与后续
   `bugate-role lineage-status` 报告的 integrity `migration_required` 不是同一状态。
5. 在临时 UC 上验证：角色 unset 和 wrong role 都拒绝；直接改
   `00_role_evidence/**` 拒绝；Memory 不可用时 strict transition 非零且无本地 receipt。
6. Engine `apply`/`verify` 成功只证明 installed projection；它不代表任何现有 UC
   接受了 lineage migration。Profile 进入 required mode 后，对每个 governed UC 运行
   `lineage-status --json`，并按本 SOP 的 §F 独立完成 init/adopt/recover；在全部目标 UC
   达到 `aligned` 前，不得把 updater 成功写成 Wave 7 lineage migration 成功。
...
`--init --auto` 会被拒绝；必须分开执行。对真实新 UC，脚手架完成后先读取
`lineage-status --json`；首次 init 前它以非零返回 `uninitialized` 是预期结果。
Operator 必须确认确属首次使用，复制输出中的 exact `lineage_id`，再显式执行
`lineage-init`。普通 lifecycle publisher 不会代做这个决定。

`lineage-init` 会在任何 Memory request 前先写一笔 durable initialization intent，并按
`pending` → `root_absence_verified` → `root_verified` →
`registry_initialized` → `chain_written` → `completed` 推进。Required 与 best-effort
都走完整 journal；best-effort 不创建 remote root/checkpoint。中断后 JSON 状态会是
`recovery_pending` 且包含 `active_initialization`；用**相同 exact ID**重跑
`lineage-init`，它会从原 stage 继续。不要对此运行 `recover`。初始 `pending` probe
若发现 strict root，命令会 abort 尚未创建 root/lineage 的 intent 并报告
`migration_required`；这不是可重试的空初始化。Intent 完成前普通 publisher 都会拒绝。

若 `lineage-status` 显示已有非空 legacy chain 的 `migration_required`，禁止运行
`lineage-init`；完整核验 chain 后按 §F 使用 `lineage-adopt`。若显示 registered
history missing/diverged 或 pending transaction，则先按 §F recovery。只有 integrity
达到 `aligned` 后才运行 `--auto`。`--auto` 结束后读取明确状态，通常先到
`READY_FOR_HUMAN_ACCEPTANCE`。Peer bridge 可生成 pending 03B，但 agent 不得把它
自行改成 passed。
...
`integrity_state` 与 lifecycle state 分开判断。先保存完整 JSON 输出、registry expected
head 与可信 recovery source，再按下表处理；不得通过删除 evidence、registry 或 Memory
home 让状态“看起来更干净”。

| `integrity_state` | Operator action |
|---|---|
| `uninitialized` | 仅在确认真实首次使用后，复制 exact `lineage_id` 并执行 `lineage-init`。 |
| `aligned` | 可继续当前 lifecycle；不需要迁移或恢复。 |
| `migration_required` + 本地非空 verified legacy chain | 用 exact chain head 执行 `lineage-adopt`；既有 receipt bytes 重写数必须为 0。 |
| `migration_required` + 只有 strict root、无可验证本地历史 | 恢复可信 pre-loss evidence 后再 adoption/restoration；空目录不能推断旧历史，禁止 init。 |
| `recovery_pending` + `active_initialization` | 用相同 exact `lineage_id` 重跑 `lineage-init`，从 journal stage 继续；不要运行 `recover`。 |
| `history_missing` / `history_diverged`，或 `recovery_pending` + active publication/recovery transaction | 用 registry 的 exact expected head 执行 `recover`；`EMPTY` 只允许表示 sequence-zero expected head。 |
| `registry_unavailable` | 停止 lifecycle write，修复 registry 或显式 root probe 的可读/校验问题后重新分类。 |

```bash
# Existing verified non-empty pre-v0.4.3 chain:
bin/bugate-role lineage-adopt <artifact-dir> \
  --lineage-id <exact-lineage-id> --expected-head <exact-chain-head>

# Registered missing/diverged history or active publication/recovery transaction:
...
Required Memory recovery 默认从 immutable checkpoint chain 还原 exact receipt/chain
bytes。显式提供 trusted archive 只选择候选 bytes；retained strict checkpoint 仍必须
可用且保持权威，每个 archive envelope 都必须在任何写入前与其精确一致。该 archive
不是 strict Memory 不可用或分歧时的离线回退。Best-effort 没有 remote
checkpoint，因此本地历史丢失时必须用 `--archive` 提供独立保留、可信的
`bugate.role-recovery-archive/v1`。完整 validation 与 write preflight 通过后，Recovery
先选定 active source 或创建 pending `recovery_restore`，独占 claim 这笔 exact source，
再恢复 committed predecessor；dead claimant 只有经过 liveness check 与 exact-token
CAS 才可 takeover。若原 publication 已到 checkpoint verified/ready-for-CAS 等可继续
stage，recovery 会让**同一笔** transaction 完成 CAS、local receipt 与 chain
publication。随后 registry 在同一个 SQLite transaction 中 terminalize 已 claim source
（`recovery_restore` 变为 `aborted`，lifecycle source 变为 `completed`），并安装唯一
canonical-bound、pending 的 `evidence_recovery` successor，不暴露
`aligned`/no-audit crash gap。若 active source 已是 `evidence_recovery`，retry 会直接
继续它，绝不再安装 successor 或生成重复的 N+1 lifecycle/recovery receipt。
Best-effort journal 已经明确记录 unanchored 空 transition ID 时，recovery 保留该
marker，不重试 HTTP。整个过程不是静默 reset，也不重写旧 receipt。

Lifecycle content drift 与 lineage-integrity recovery 也必须分开：profile/pre-code drift
重做 human acceptance + designer handoff + implementer acceptance；implementation drift
重做 implementer handoff + reviewer acceptance。两者都追加 superseding generation，
不得删除 evidence reset。

### G. Operator/network 与安全边界

- 普通 `status`、hook、per-edit preflight 只查本地 registry/receipt/hash，Memory HTTP
  请求数必须为零。不要把 remote availability 放进每次编辑的热路径。
- `lineage-status` 是显式 operator diagnostic；仅在 required Memory 且 local state 看似
  `uninitialized` 时 exact-GET deterministic root。Root 存在则非零
  `migration_required`；probe 失败则非零 `registry_unavailable`。
- Required-mode `lineage-init` 重复该 root probe；required-mode `lineage-adopt`/
  `recover` 也访问 strict Memory。普通 lifecycle transition 在两种模式下都可能尝试
  Memory anchor：required 失败关闭，best-effort 容忍失败。这些是显式 operator/
  transition 边界，不是 hook 行为。
- `required` 建立 deterministic root 与 immutable checkpoint；`best_effort` 仍用
  machine registry 做 deletion detection/CAS，不创建 remote root/checkpoint，也不能
  从远端恢复丢失的本地历史。
- 环境变量、hook、本地文件、registry 与 Memory 只提供可审计声明和 tamper/drift
  detection，不提供不可抵赖身份。
- Hook 不拦截任意 shell 重定向、递归删除或外部编辑器。强隔离需要 OS 账号、容器、
  managed runner、受保护备份或按角色凭据。同一 OS user 若同时删除 workspace evidence、
  machine registry 与整个 Memory home，超出本地威胁边界。
