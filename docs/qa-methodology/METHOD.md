...
changelog:
  v1.0 → v1.1:
    - 新增 Wave 0：PRD 健康度体检（含 9 维度评估与缺口报告输出）
...
|---|---|---|---|
| Wave 0 PRD 健康度体检 | 9 维准入门禁 + 缺口报告 | 需求可测性 / 需求的静态测试("testable requirements") | ISO/IEC/IEEE 29148(系统与软件需求工程);ISTQB 基础级"静态测试" |
...
但现实中"PRD 质量差"是连续光谱而不是二元状态。绝大多数项目的 PRD 都在中间地带：核心流程清楚、异常路径模糊、边界值缺失、新旧规则混杂。Wave 0 的任务是**用 9 个机械可执行的维度量化 PRD 的可用性**，并据此分流。
...
```
project-root/
├── AGENTS.md                          # agent 协议；与 .shared/ 组成开发态 fallback 哨兵
...
1. 接入 BUGate 引擎（导入模式：vendor / 子模块进 SUT 测试仓；开发 BUGate 本身：直接用引擎仓）。根定位已拆分：工作区根 = 自 CWD 向上最近的 `bugate.config.yaml`（`AGENTS.md` + `.shared/` 哨兵为开发态 fallback），模板等引擎资产按引擎自身位置解析。
