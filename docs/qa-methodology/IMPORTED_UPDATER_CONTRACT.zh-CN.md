...
更新器不得写入、删除、stage、commit 或格式化其他任何 surface。这包括
`bugate.config.yaml`、`bugate.profile.yaml`、`docs/usecases/**`、
`00_role_evidence/**`、human acceptance artifact、SUT
...
v0.4.3 lineage 修正案增加了另一道独立 operator boundary。更新器可以安装
具备 lineage 能力的 engine files，但绝不创建或编辑 machine-level lineage registry，
不发布 deterministic lineage root/checkpoint，也不运行 `bugate-role lineage-init`、
`lineage-adopt` 或 `recover`。因此 engine `apply`/`verify` 成功**不等于**接受
per-UC lineage migration。Engine update 经审查并提交后，operator 必须对每个受治理
UC 运行 `bugate-role lineage-status <artifact-dir> --json`，再分别确认 exact
initialization、adoption 或 recovery 路径。Updater 的 profile compatibility
`migration_available`/`migration_required` 与 `bugate-role` 的 history-integrity
`migration_required` 是不同信号，禁止混淆。
...
更新器不得重建、清空、迁移或写入 Memory Bus data 或 lineage registry data，只可作
read-only health check。Memory downtime 必须单独报告：它不能把已落盘的 engine
update 误称为已完全验收的 strict-role transition。required Memory transition 仅在
操作者之后 activation/verification governance 时 fail-closed。

Updater rollback 只恢复其 manifest-owned engine transaction，不会撤销独立接受的
profile change、lineage adoption、registry record、Memory root/checkpoint 或 lifecycle
receipt。这些操作需要自己的 compatibility review 与 recovery record，绝不能藏入
updater transaction。
...
对具备 lineage 能力的 release，ownership-preservation acceptance 还必须 snapshot
effective Memory home，并证明 updater plan/apply/verify/rollback 不创建或修改
`role-lineage.sqlite3`、lineage root、checkpoint、profile、namespace 或 role evidence。
安装 command surface 不能替代其独立的 SUT-neutral lineage init/adopt/recovery 契约验收。
...
verification、recovery-augmented strict Memory 契约（六个普通 lifecycle event
加一条保持状态的 `evidence_recovery` receipt，正式 smoke route 共七个 exact
anchor）、bootstrap/apply、幂等、rollback/reapply 与更新后的
ownership-preservation oracle。独立的
