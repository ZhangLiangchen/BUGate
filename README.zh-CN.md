...
Codex。该 prompt 会先区分首次安装、v0.3.x bootstrap 与 v0.4+ 仓内更新，再引导
...
| 无 updater 的受支持 v0.3.x import | 已解包 v0.4.2 的 `scripts/bugate_update.py` | 一次性 bootstrap；禁止 rerun installer。 |
| 带 `.bugate/bin/bugate-update` 的 v0.4+ import | vendored `bugate-update` | `status` → `plan` → 人工复核后的 `apply` → `verify`；回滚使用 exact transaction id。 |
...
受支持的 v0.3.x 安装没有 vendored updater，所以只用一次已解包 v0.4.2 release
里的 bootstrap updater：
...
v0.4+ 安装使用仓内入口，并始终显式给出目标版本；不存在隐式 `latest`：
...
若要撤销一个已提交 transaction，使用 `apply` 输出的 exact 32-hex id，然后验证：
