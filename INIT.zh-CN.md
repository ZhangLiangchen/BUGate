...
>
> 当前 checkout 的 release 版本线是 **v0.4.5**。只有 annotated tag 与 GitHub
> Release 已公开且仅公开 tar、zip、`bugate-0.4.5.SHA256SUMS` 三项资产，并且所选
> archive 校验通过后，才把它视为当前已发布版本；在此之前，当前已发布回退版本仍是
> v0.4.4（见 `IMPORT_PROMPT.zh-CN.md`）。
...
`--archive /outside/bugate-0.4.5.tar.gz --checksums
/outside/bugate-0.4.5.SHA256SUMS`。`status`、`plan`、`verify` 都是只读的；
...
命名空间来自 SUT profile（`memory.namespace`）或 `MEMORY_BUS_PROJECT_TAG`（默认 `project:bugate`）。服务是**机器级**的（ADR-BUGATE-003）：全机一个实例，数据家目录 `~/.bugate/memory-bus/`（用 `BUGATE_MEMORY_HOME` 覆盖；服务自身的 `MCP_MEMORY_BASE_DIR` 优先级最高），被本机所有被治理仓共享、项目间靠 namespace tag 隔离 —— 被治理仓只在 profile 里声明其 namespace，**不**脚手架本地服务目录。仓内遗留 `.memory_bus/` 仍作为弃用回退被读取。可选 macOS 加固：`bin/memory-bus-install-launchd`（RunAtLoad + KeepAlive；`--uninstall` 卸载）。记忆总线是**必要核心组件**：`bugate init` / `bin/memory-bus-*` 缺席则**自动安装**机器级服务、异常则**自愈拉起**。普通 recall/note/Stop 与每次编辑继续 best-effort/本地校验；Wave 7 `memory_mode: required` 下，临时故障会阻塞下一次 strict operator/transition boundary——包括 lineage probe/init/adopt/recover 与 approve/handoff/accept/complete——且不会产生已完成的本地解锁 publication。`BUGATE_MEMORY_NO_INSTALL=1` 可在离线/受限机器跳过自动安装。
...
普通编辑只验证本地 registry/hash chain；strict Memory 故障会阻塞下一次转换，恢复后可
