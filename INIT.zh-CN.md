...
- **使用者路径 —— 导入模式（默认）。** 你要用 BUGate 治理某个 SUT 的自动化测试仓：
  先按下文完成核心验证，然后按 README「Quickstart A) Imported mode」把引擎 +
  skill 装进 **SUT 测试仓**、在那边接线 hooks，并把 `bugate.config.yaml` +
  profile **提交进 SUT 仓**。日常 agent 会话打开的是 **SUT 测试仓**，不是本仓。
...
5. **memory-bus 建议固定在项目 `.venv`。** 新版 `mcp-memory-service` 启动时会实际用到
...
5. 验证 memory-bus:
   ...
   - find ~/.cache/mcp_memory/onnx_models -name '*.onnx' -print
   - MCP_MEMORY_BASE_DIR="$PWD/.memory_bus" MCP_MEMORY_STORAGE_BACKEND=sqlite_vec
